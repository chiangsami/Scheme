...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        env.define(signature,scheme_eval(expressions.rest.first,env,None))
        return signature
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        vals = Pair(signature.rest,expressions.rest)
        env.define(signature.first,do_lambda_form(vals,env))
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return is_scheme_true(scheme_eval(expressions,env,True))
    while expressions is not nil:
        t_or_f=scheme_eval(expressions.first,env,)
        if is_scheme_false(t_or_f):
            return t_or_f
        else:
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        t_or_f=scheme_eval(expressions.first,env,expressions.rest==nil)
        if is_scheme_true(t_or_f) or expressions.rest==nil:
            return t_or_f
        else:
            expressions=expressions.rest
    return False
